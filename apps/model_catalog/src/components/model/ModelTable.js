...
import { formatAuthors, downloadJSON, showNotification } from "../../utils/utils";
import MUIDataTableCustomToolbar from "../layout/MUIDataTableCustomToolbar";
import CustomToolbarSelect from "../layout/MUIDataTableCustomRowToolbar";
...
class ModelTable extends React.Component {
    ...
    hideTableRows(selectedRows) {
        ...
        showNotification(
            this.props.enqueueSnackbar,
            this.props.closeSnackbar,
            "Chosen model(s) have been hidden!",
            "info"
        );
    }
            ...
            if (model.instances.length > 0) {
                ...
                for (let model_inst of model.instances) {
                    ...
                    if (
                        !(
                            model_inst.id in
                            compareModels[model.id].selected_instances
                        )
                    ) {
                        compareModels[model.id].selected_instances[
                            model_inst.id
                ...
                showNotification(
                    this.props.enqueueSnackbar,
                    this.props.closeSnackbar,
                    "Model '" + model.name + "' added to compare!",
                    "info"
                );
            } else {
                showNotification(
                    this.props.enqueueSnackbar,
                    this.props.closeSnackbar,
                    "Skipped: model '" + model.name + "' (0 instances)!",
                    "error"
                );
            }
        }
                        ...
                        options={{
                            ...
                            customToolbar: () => {
                                return (
                                    <MUIDataTableCustomToolbar
                                        ...
                                        tableType="models"
                                        addNew={this.props.openAddModelForm}
                                    />
                                );
                            },
                            customToolbarSelect: (selectedRows) => (
                                <CustomToolbarSelect
                                    ...
                                    addCompare={this.addModelCompare}
                                />
                            ),
                        }}
                    />
