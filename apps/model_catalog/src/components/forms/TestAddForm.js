...
import { replaceEmptyStringsWithNull } from "../../utils/utils";
import LoadingIndicatorModal from "../layout/LoadingIndicatorModal";
...
export default class TestAddForm extends React.Component {
    ...
    createPayload() {
        ...
        if (
            !Object.values(payload.instances[0]).some((element) =>
                Boolean(element)
            )
        ) {
            payload.instances = [];
    ...
    handleSubmit() {
        this.setState({ loading: true }, () => {
            ...
            if (this.checkRequirements(payload)) {
                datastore
                    .createTest(payload, this.signal)
                    .then((test) => {
    ...
    render() {
        ...
        if (this.state.errorAddTest) {
            errorMessage = (
                <ErrorDialog
                    open={Boolean(this.state.errorAddTest)}
                    handleErrorDialogClose={this.handleErrorAddDialogClose}
                    error={
                        this.state.errorAddTest.message ||
                        this.state.errorAddTest
                    }
                />
            );
        }
