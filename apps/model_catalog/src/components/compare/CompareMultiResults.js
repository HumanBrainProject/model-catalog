...
import { formatTimeStampToCompact, roundFloat } from "../../utils/utils";
                            ...
                            <StyledTableRow key={model_inst_id}>
                                ...
                                </TableCell>
                                {
                                    ...
                                    list_tests.map(function (test) {
                                        return dict_test_versions[
                                            test.test_id
                                        ].map(function (test_inst) {
                                            if (
                                                Object.keys(
                                                    result_model
                                                        .model_instances[
                            ...
                            <StyledTableRow key={test_inst_id}>
                                ...
                                </TableCell>
                                {
                                    ...
                                    list_models.map(function (model) {
                                        return dict_model_versions[
                                            model.model_id
                                        ].map(function (model_inst) {
                                            if (
                                                Object.keys(
                                                    result_test.test_instances[
                                                        test_inst_id
...
class CompareMultiResults extends React.Component {
    ...
    evalModelDict() {
        ...
        for (let m_key in this.state.model_dict) {
            model_inst_ids = model_inst_ids.concat(
                Object.keys(this.state.model_dict[m_key].selected_instances)
            );
        }
    ...
    evalTestDict() {
        ...
        for (let t_key in this.state.test_dict) {
            test_inst_ids = test_inst_ids.concat(
                Object.keys(this.state.test_dict[t_key].selected_instances)
            );
        }
    ...
    fetchModelResults() {
        return datastore
            .getResultsByModelInstances(this.state.model_inst_ids)
            .then((results) => {
                this.setState({
    ...
    fetchTestResults() {
        return datastore
            .getResultsByTestInstance(this.state.test_inst_ids, this.signal)
            .then((results) => {
                this.setState({
                    loadingResults: false,
                });
    ...
    getModelVersions(results) {
        ...
        for (let result of results) {
            ...
            if (
                dict_model_versions[result.model.id]
                    .map((item) => item.model_inst_id)
                    .indexOf(result.model_instance.id) === -1
            ) {
    ...
    getTestVersions(results) {
        ...
        for (let result of results) {
            ...
            if (
                dict_test_versions[result.test.id]
                    .map((item) => item.test_inst_id)
                    .indexOf(result.test_instance.id) === -1
            ) {
    ...
    addEmptyResults(results_grouped) {
        ...
        Object.keys(model_dict).forEach(function (m_key, index) {
            ...
            Object.keys(model_dict[m_key].selected_instances).forEach(function (
                m_inst_key,
                index
            ) {
                //
                ...
                if (
                    !(m_inst_key in results_grouped[m_key]["model_instances"])
                ) {
                    results_grouped[m_key]["model_instances"][m_inst_key] = {
                        model_inst_id: m_inst_key,
                        model_version:
                            model_dict[m_key].selected_instances[m_inst_key]
                                .version,
                        timestamp:
                            model_dict[m_key].selected_instances[m_inst_key]
                                .timestamp,
                ...
                Object.keys(test_dict).forEach(function (t_key, index) {
                    ...
                    Object.keys(test_dict[t_key].selected_instances).forEach(
                        function (t_inst_key, index) {
                            //
                            ...
                            if (
                                ...
                                results_grouped[m_key]["model_instances"][
                                    ...
                                    test_version:
                                        test_dict[t_key].selected_instances[
                                            t_inst_key
                                        ].version,
                                    timestamp:
                                        test_dict[t_key].selected_instances[
            ...
            Object.keys(dict_results[model_id]["model_instances"]).forEach(
                function (model_inst_id) {
                    ...
                    Object.keys(
                        ...
                        .sort(function (a, b) {
                            var t_a_display = dict_results[model_id][
                                "model_instances"
            ...
            Object.keys(dict_results[model_id]["model_instances"]).forEach(
                function (model_inst_id) {
                    Object.keys(
                        ...
                        Object.keys(
                            ...
                            .sort(function (a, b) {
                                var t_a_timestamp =
                                    dict_results[model_id]["model_instances"][
                                        model_inst_id
                                    ]["tests"][test_id]["test_instances"][a]
                                        .timestamp;
                                var t_b_timestamp =
                                    dict_results[model_id]["model_instances"][
                                        model_inst_id
                                    ]["tests"][test_id]["test_instances"][b]
                                        .timestamp;
                                if (t_a_timestamp < t_b_timestamp) {
                                    return -1;
                                }
                        ...
                        dict_results[model_id]["model_instances"][
                            model_inst_id
                        ]["tests"][test_id]["test_instances"] = temp_sorted;
                    });
                }
            );
        });
    ...
    groupResultsTestsCompare(results) {
        ...
        Object.keys(dict_results)
            .sort(function (a, b) {
                var t_a_display = dict_results[a].test_alias
                    ? dict_results[a].test_alias
                    : dict_results[a].test_name;
                var t_b_display = dict_results[b].test_alias
                    ? dict_results[b].test_alias
                    : dict_results[b].test_name;
                if (t_a_display < t_b_display) {
                    return -1;
                }
                if (t_a_display > t_b_display) {
        ...
        Object.keys(dict_results).forEach(function (test_id) {
            ...
            Object.keys(dict_results[test_id]["test_instances"])
                .sort(function (a, b) {
                    var t_a_timestamp =
                        dict_results[test_id]["test_instances"][a].timestamp;
                    var t_b_timestamp =
                        dict_results[test_id]["test_instances"][b].timestamp;
                    if (t_a_timestamp < t_b_timestamp) {
                        return -1;
                    }
                    if (t_a_timestamp > t_b_timestamp) {
            ...
            Object.keys(dict_results[test_id]["test_instances"]).forEach(
                function (test_inst_id) {
                    ...
                    Object.keys(
                        ...
                        .sort(function (a, b) {
                            var t_a_display = dict_results[test_id][
                                "test_instances"
                        ...
                        Object.keys(
                            ...
                            .sort(function (a, b) {
                                var t_a_timestamp =
                                    dict_results[test_id]["test_instances"][
                                        test_inst_id
                                    ]["models"][model_id]["model_instances"][a]
                                        .timestamp;
                                var t_b_timestamp =
                                    dict_results[test_id]["test_instances"][
                                        test_inst_id
                                    ]["models"][model_id]["model_instances"][b]
                                        .timestamp;
                                if (t_a_timestamp < t_b_timestamp) {
                                    return -1;
                                }
    ...
    renderModelsResultsSummaryTable(
        dict_results,
        list_tests,
        dict_test_versions
    ) {
                                ...
                                <StyledTableRow>
                                    ...
                                    {list_tests.map((test, index) => (
                                        <TableCell
                                            align="center"
                                            colSpan={
                                                dict_test_versions[test.test_id]
                                                    .length * 3
                                            }
                                            key={test.test_id}
                                            bgcolor={Theme.darkBackground}
    ...
    renderTestsResultsSummaryTable(
        dict_results,
        list_models,
        dict_model_versions
    ) {
                        ...
                        >
                            ...
                            <TableBody>
                                {Object.keys(dict_results).map(
                                    (test_id, index_t) => (
                                        <ResultEntryTest
                                            result_entry={dict_results[test_id]}
    ...
    launchCompare() {
        ...
        if (this.state.compareShow === "common_models") {
            ...
            for (let t_inst_key of test_inst_ids) {
                ...
                if (
                    t_inst_for_m_inst.sort().toString() ===
                    this.state.model_inst_ids.sort().toString()
                ) {
            ...
            for (let m_inst_key of model_inst_ids) {
                ...
                if (
                    m_inst_for_t_inst.sort().toString() ===
                    this.state.test_inst_ids.sort().toString()
                ) {
        ...
        if (required_results.length === 0) {
            ...
            if (this.state.compareShow === "common_tests") {
                const [list_models, dict_model_versions] =
                    this.getModelVersions(required_results);
                results_grouped =
                    this.groupResultsTestsCompare(required_results);

                content_table = this.renderTestsResultsSummaryTable(
                    results_grouped,
                    list_models,
                    dict_model_versions
                );
                ...
                const [list_tests, dict_test_versions] =
                    this.getTestVersions(required_results);
                results_grouped =
                    this.groupResultsModelsCompare(required_results);
                ...
                content_table = this.renderModelsResultsSummaryTable(
                    results_grouped,
                    list_tests,
                    dict_test_versions
                );
    ...
    handleModelClick(m_key) {
        if (this.state.open_models.indexOf(m_key) > -1) {
            this.setState({
                open_models: this.state.open_models.filter(
                    (item) => item !== m_key
                ),
            });
        } else {
    ...
    handleTestClick(t_key) {
        if (this.state.open_tests.indexOf(t_key) > -1) {
            this.setState({
                open_tests: this.state.open_tests.filter(
                    (item) => item !== t_key
                ),
            });
        } else {
    ...
    handleModelDeleteClick(m_key, m_inst_key) {
        ...
        if (Object.keys(model_dict).includes(m_key)) {
            if (
                Object.keys(model_dict[m_key].selected_instances).includes(
                    m_inst_key
                )
            ) {
                delete model_dict[m_key].selected_instances[m_inst_key];
                if (
                    Object.keys(model_dict[m_key].selected_instances).length ===
                    0
                ) {
                ...
                results = results.filter(function (result) {
                    return result.model_instance.id !== m_inst_key;
                });
                this.setState({
    ...
    handleTestDeleteClick(t_key, t_inst_key) {
        ...
        if (Object.keys(test_dict).includes(t_key)) {
            if (
                Object.keys(test_dict[t_key].selected_instances).includes(
                    t_inst_key
                )
            ) {
                delete test_dict[t_key].selected_instances[t_inst_key];
                if (
                    Object.keys(test_dict[t_key].selected_instances).length ===
                    0
                ) {
                ...
                results = results.filter(function (result) {
                    return result.test_instance.id !== t_inst_key;
                });
                this.setState({
                    test_dict: test_dict,
                    results: results,
        ...
        return (
            <Grid item xs={12} align="center">
                <Typography variant="h6">
                    <br />
                    There are no validation results matching the specified
                    criteria!
                </Typography>
            </Grid>
        );
    ...
    clearTests() {
        ...
        Object.keys(test_dict).forEach(function (t_key) {
            Object.keys(test_dict[t_key].selected_instances).forEach(function (
                t_inst_key
            ) {
                delete test_dict[t_key].selected_instances[t_inst_key];
                results = results.filter(function (result) {
    ...
    render() {
        ...
        if (this.state.loadingResults && this.state.compareShow) {
            ...
            [content_table, content_figures, content_summary] =
                this.launchCompare();
        }
                                                ...
                                                <List dense={true} style={{
                                                    ...
                                                    >
                                                        ...
                                                        <ListItemText
                                                            ...
                                                            secondary={m_key}
                                                        />
                                                        {this.state.open_models.indexOf(
                                                            m_key
                                                        ) > -1 ? (
                                                            <ExpandLess />
                                                        ) : (
                                    ...
                                    <Grid item xs={12} align="center">
                                        ...
                                        >
                                            <Typography variant="subtitle1">
                                                ...
                                                <strong>Note: </strong>
                                                <br />
                                                If no tests are selected, then
                                                all validation tests associated
                                                with the selected models will be
                                                automatically chosen.
                                                <br />
                                                Similarly, if no models are
                                                selected, then all models
                                                associated with the selected
                                                tests will be automatically
                                                chosen.
                                            </Typography>
                                        </Box>
                                    </Grid>
                                    ...
                                    <Grid item xs={12} align="center">
                                        ...
                                        >
                                            <Typography variant="subtitle1">
                                                <br />
                                                <strong>Note: </strong>
                                                Since no models were selected,
                                                all models associated with the
                                                selected tests will be
                                                automatically chosen.
                                            </Typography>
                                        </Box>
                                    </Grid>
                        ...
                        {this.state.compareShow === "common_models" && (
                            ...
                            >
                                <Grid item xs={12} align="center">
                                    <Typography variant="subtitle1">
                                        <strong>
                                            Showing only results associated with
                                            validation test instances common to
                                            all model instances!
                                        </strong>
                                    </Typography>
                                </Grid>
                            ...
                            >
                                <Grid item xs={12} align="center">
                                    <Typography variant="subtitle1">
                                        <strong>
                                            Showing only results associated with
                                            model instances common to all
                                            validation test instances!
                                        </strong>
                                    </Typography>
                                </Grid>
                            </Grid>
