...
function ResultsFiguresSummaryXaxisModels(props) {
    ...
    for (let test_id in dict_results) {
        for (let test_inst_id in dict_results[test_id]["test_instances"]) {
            ...
            var customdata = [];
            for (let model_id in dict_results[test_id]["test_instances"][
                test_inst_id
            ]["models"]) {
                for (let model_inst_id in dict_results[test_id][
                    "test_instances"
                ][test_inst_id]["models"][model_id]["model_instances"]) {
                    ...
                    let result_entry =
                        dict_results[test_id]["test_instances"][test_inst_id][
                            "models"
                        ][model_id]["model_instances"][model_inst_id][
                            "results"
                        ][0];
                    model_labels.push(
                        dict_results[test_id]["test_instances"][test_inst_id][
                            ...
                            ? dict_results[test_id]["test_instances"][
                                  test_inst_id
                              ]["models"][model_id]["model_alias"]
                            : dict_results[test_id]["test_instances"][
                                  test_inst_id
                              ]["models"][model_id]["model_name"]
                    );
                    model_version_labels.push(
                        dict_results[test_id]["test_instances"][test_inst_id][
                            "models"
                        ][model_id]["model_instances"][model_inst_id][
                            "model_version"
                    ...
                    label_resultJSON_map[result_entry["result_id"]] =
                        result_entry.result_json;
                }
            }
            traces.push({
                x: [model_labels, model_version_labels],
                y: model_version_scores,
                // text:model_labels,
                // customdata is used for setting hover description
                customdata: customdata,
                hovertemplate:
                    "Model: <b>%{customdata[0]}</b><br>" +
        ...
        <Plot
            ...
            onClick={(data) =>
                props.handleResultEntryClick(
                    label_resultJSON_map[data["points"][0]["customdata"][4]]
                )
            }
            config={{
                displaylogo: false,
...
function ResultsFiguresSummaryXaxisTests(props) {
    ...
    for (let model_id in dict_results) {
        for (let model_inst_id in dict_results[model_id]["model_instances"]) {
            ...
            var customdata = [];
            for (let test_id in dict_results[model_id]["model_instances"][
                model_inst_id
            ]["tests"]) {
                for (let test_inst_id in dict_results[model_id][
                    "model_instances"
                ][model_inst_id]["tests"][test_id]["test_instances"]) {
                    ...
                    let result_entry =
                        dict_results[model_id]["model_instances"][
                            model_inst_id
                        ]["tests"][test_id]["test_instances"][test_inst_id][
                            "results"
                        ][0];
                    test_labels.push(
                        dict_results[model_id]["model_instances"][
                            model_inst_id
                        ]["tests"][test_id]["test_alias"]
                            ? dict_results[model_id]["model_instances"][
                                  model_inst_id
                              ]["tests"][test_id]["test_alias"]
                            : dict_results[model_id]["model_instances"][
            ...
            traces.push({
                x: [test_labels, test_version_labels],
                y: test_version_scores,
                // text:test_labels,
                // customdata is used for setting hover description
                customdata: customdata,
                hovertemplate:
                    "Model: <b>%{customdata[2]}</b><br>" +
...
export default class CompareMultiGraphs extends React.Component {
    constructor(props) {
        ...
        this.groupResults_XaxisModels =
            this.groupResults_XaxisModels.bind(this);
                ...
                <Grid container>
                    ...
                    <Grid item xs={12} align="center">
                        <Box
                            ...
                            justifyContent="center"
                            alignItems="center"
                            style={{ maxWidth: "900px", marginBottom: 25 }}
                        >
                            <Typography variant="subtitle1">
                                <br />
                                <strong>Note: </strong>
                                Summary plots only display the latest result for
                                each combination of model instance and test
                                instance.
                            </Typography>
                        </Box>
                    </Grid>
